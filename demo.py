...
def main():
    ...
    try:
        ...
        if GITHUB_TOKEN:
            ...
            # Use Agent to post the comment manually
            print("           Using Agent to post comment (no token provided)")
            pipeline.add_stage(
                Destination(
                    url=GITHUB_ISSUE_COMMENT_URL,
                    method="AGENT",  # Agent to post manually
                    body_template='{"body": "## PowerFlow AI Code Insights Report\\n\\nHere are some automatically extracted code snippets for review:\\n\\n```json\\n$DATA\\n```\\n\\n---\\n*Generated by PowerFlow AI Reporter*"}'
                )
            )
        ...
        print("\n" + "="*80)
        if result.success:
            print("✅ AI Code Insight analysis complete!")
            ...
            if not GITHUB_TOKEN:
                print("\n   💡 Note: Comment posted via Agent (no token required)")
        else:
            print("❌ Code insight analysis failed with errors:")
            for error in result.errors:
                print(f"   • {error}")
        print("="*80)
        
        print("\n📊 Pipeline Statistics:")
        print(f"  • Records processed: {result.metadata.get('record_count', 0)}")
