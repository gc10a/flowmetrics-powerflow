...
🔌 **Built-in Integrations** - Connect to GitHub, Salesforce, HubSpot, and more out of the box
...
PowerFlow includes an AI-powered feature for analyzing GitHub repositories to identify key patterns and automatically report insights. Using the GitHub connector API, you can search any public repository, extract code snippets, and generate reports - no GitHub token required! This powerful capability aids in continuous code quality improvement and security auditing.
...
GITHUB_REPO = "YOUR_USERNAME/YOUR_REPOSITORY_NAME"  # Repository to scan
...
# If GITHUB_TOKEN is set, uses direct API; otherwise, the Agent posts the comment manually.
...
if GITHUB_TOKEN:
    ...
    # Agent will need to post the comment manually using browser actions after calling the search() tool from GitHub Connectors
    pipeline.add_stage(
        Destination(
            url=GITHUB_ISSUE_COMMENT_URL,
            method="AGENT",
...
if result.success:
    print("✅ AI Code Insight reporting complete!")
...
1. **Sources** - Read data from various inputs (GitHub Connector, CSV, JSON, Salesforce, HubSpot, etc.)
...
- `GitHubConnectorScannerSource` - Search GitHub repositories using the connector API
...
- 📖 [Documentation](docs/)
- 💬 [GitHub Discussions](https://github.com/gc10a/flowmetrics-powerflow/discussions)
- 🐛 [Issue Tracker](https://github.com/gc10a/flowmetrics-powerflow/issues/)
- 🌟 [Examples](examples/)
